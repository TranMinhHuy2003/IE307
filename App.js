...
const App = () => {
  const [listItems, setListItems] = useState(flowers);
  ...
  const hoachons = flowers.filter(layhoatheoloai);
  ...
  return (
    ...
    >
      ...
      <Picker.Item label = "Hoa Tình Yêu" value = "Hoa Tình Yêu" />
    </Picker>
      <Modal
        animationType={'slide'}
        transparent={false}
        ...
        <View style={styles.modal}>
          ...
          <Text style={styles.text}>Tên Loại :{hoachon.tenloai}</Text>
          <Text style={styles.text}>Đơn Giá :{hoachon.dongia}</Text>
          <Image 
              source={hoachon.hinh}  
              style={{width: 200, height: 200}} 
          />
          <Text style={styles.text}>Mô tả {hoachon.mota}</Text> 
          <Button
            title="Close"
            onPress={() => {
              setShowModal(!showModal);
